...
resource "google_secret_manager_secret_version" "db_user_password_v1" {
  ...
  # depends_on = [google_secret_manager_secret_iam_member.terraform-as-admin]
}

## Secrets that frontend needs
# Secret for artifact registry image path frontend
resource "google_secret_manager_secret" "artifact_registry_image_path_frontend" {
  secret_id = "${var.environment}-artifact-registry-frontend-image-path"
  replication {
    user_managed {
      replicas {
        location = var.project_region
...
resource "google_secret_manager_secret_version" "artifact_registry_image_path_frontend_v1" {
  secret      = google_secret_manager_secret.artifact_registry_image_path_frontend.id
  secret_data = var.artifact_registry_image_path_frontend_v1
}

# Secret for cloud run frontend service name
resource "google_secret_manager_secret" "cloud_run_frontend_service_name" {
  secret_id = "${var.environment}-cloud-run-frontend-service-name"
  replication {
    user_managed {
      replicas {
        location = var.project_region
...
resource "google_secret_manager_secret_version" "cloud_run_frontend_service_name_v1" {
  secret      = google_secret_manager_secret.cloud_run_frontend_service_name.id
  secret_data = var.cloud_run_frontend_service_name_v1

}

# Secret as backend URL for frontend to call
resource "google_secret_manager_secret" "backend_service_url" {
  secret_id = "${var.environment}-backend-service-url"
  replication {
    user_managed {
      replicas {
        location = var.project_region
...
resource "google_secret_manager_secret_version" "backend_service_url_v1" {
  secret      = google_secret_manager_secret.backend_service_url.id
  secret_data = var.cloud_run_backend_service_url_v1
